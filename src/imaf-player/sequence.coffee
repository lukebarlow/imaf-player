...
Track = require('prong/track/track')
Timeline = require('prong/components/timeline')
...
# TODO : make a way to do this without importing the whole of prong
prong = require('prong')
prong.registerTrackType('audio', require('./groupedAudioTrack'));
...
module.exports = ->
    ...
    audioOut = audioContext().destination # a web audio API node which  audio from this sequence will play out of
    trackHeight = null # the default height in pixels for tracks (can be overriden for specific tracks)
    trackLoadCount = 0
    timelineHeight = 40
    waveformVerticalZoom = 1
    pool = null
    dispatch = d3.dispatch('scrub','change','play','stop','tick','load','volumeChange','expand', 'contract')
    scrollZone = null
    timeline = null
    musicalTime = null
    zoomable = true
    following = true # toggle for whether the display updates to follow playback
    propertyPanelWidth = 60
    rightMargin = 0
    ...
    buildX = (container, tracks) ->
        width = container.property('clientWidth') - propertyPanelWidth - rightMargin
        duration = d3.max tracks, (track) ->
            track._channel.length / track._buffer.sampleRate
        return d3.scale.linear().domain([0, duration]).range([0, width])
    ...
    sequence = (_element) ->
        ...
        # TODO : put all the expanding stuff into a separate module
        x = null
        y = null
        w = null
        h = null
        ...
        pp = PropertyPanel().sequence(sequence)
            .on 'openRemix', ->
                [x, y] = elementPosition(element)
                w = element.node().clientWidth
                h = element.node().clientHeight
                
                dispatch.expand(1000)
                ...
                element
                    ...
                    .style('height', h + 'px')
                    .style('width', w + 'px')
                    .style('background-color', 'rgba(240, 240, 240, 0)')
        ...
        container = element.append('div')
            ...
            .style('width', "calc(100% - #{propertyPanelWidth + rightMargin}px)")
